...
class ImageDownloader {
  ...
  static Future<bool> downloadAsImage(Bingo bingo) async {
    final res = await Option.async<Uint8List>(($) async {
      final currentContext = Option.from(downloadKey.currentContext)[$]; // returns early if null
      final renderObject =
          Option.from(currentContext.findRenderObject() as RenderRepaintBoundary?)[$]; // returns early if null

      final image = await renderObject.toImage();
      final byteData = Option.from(await image.toByteData(format: ImageByteFormat.png))[$]; // returns early if null
